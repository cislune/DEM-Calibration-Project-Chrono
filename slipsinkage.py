...
NUM_TRIALS = 1
# number of independent trial groups to run
# each trial contains the full set of slip cases defined in c.SLIP_VALUES_st
...
for trial_num in range(NUM_TRIALS):
    ...
    for slip_value in c.SLIP_VALUES_st:
        ...
        # ensures terrain geometry consistency across slip cases
        ...
        # create parent output directory for this specific slip case inside the current trial
        # directory structure groups all slip cases under the same trial

        terrain_motion_dir = os.path.join(out_dir, "terrain motion")
        wheel_motion_dir = os.path.join(out_dir, "wheel motion")
        contact_forces_dir = os.path.join(out_dir, "contact forces")
        settled_data_dir = os.path.join(out_dir, "settled data")
        # create dedicated subdirectories for each output type
        # keeps terrain motion, wheel motion, contact data, and final settled state separated
        ...
        # ensure all output-type directories exist before writing files
        ...
        while t < c.TRIAL_RUN_TIME_SLIP_SINKAGE_st:
            ...
            if frame % 5 == 0:
                ...
                solver.WriteSphereFile(os.path.join(terrain_motion_dir, f"{c.SLIP_SINKAGE_TRIALS_MOTION_TERRAIN_FILE_NAME}_{frame:04d}.csv"))
                # terrain particle positions
                # used for deformation/sinkage analysis

                solver.WriteMeshFile(os.path.join(wheel_motion_dir, f"{c.SLIP_SINKAGE_TRIALS_MOTION_WHEEL_FILE_NAME}_{frame:04d}.vtk"))
                # wheel mesh motion (VTK for visualization)
                # compatible with ParaView

                solver.WriteContactFile(os.path.join(contact_forces_dir, f"{c.SLIP_SINKAGE_TRIALS_CONTACT_FORCE_FILE_NAME}_{frame:04d}.csv"))
        ...
        solver.WriteClumpFile(os.path.join(settled_data_dir, f"{c.SLIP_SINKAGE_TRIALS_SETTLED_DATA_FILE_NAME}_Slip_{slip_value:.1f}.csv"))
        # save final terrain state after slip–sinkage interaction
        # useful for post-run comparison between slip conditions within a trial
